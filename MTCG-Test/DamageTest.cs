...
namespace MTCG_Test
{
    ...
    public class DamageTest
    {
        ...
        {
            ICard player1Card = new MonsterCard("WaterDragon", rawDamage1, MonsterType.Dragon, Element.Water, new List<ISpecialty> { new EvadedByFireElf() });
            ICard player2Card = new MonsterCard("FireElf", rawDamage2, MonsterType.Elf, Element.Fire, new List<ISpecialty>());

            (float damage1, float damage2) = DamageUtility.CalculateDamage(player1Card, player2Card, _battleLog);
            Assert.Zero(damage1, "Dragon cannot hit FireElf, so zero damage");
            Assert.AreEqual(rawDamage2, damage2, $"FireElf can deal {rawDamage2} damage points, since there are no more specialties or effectiveness");
        ...
        {
            ICard player1Card = new MonsterCard("FireKraken", rawDamage1, MonsterType.Kraken, Element.Fire, new List<ISpecialty> { new ImmunAgainstSpells() });
        ...
        {
            ICard player1Card = new MonsterCard("FireDragon", rawDamage1, MonsterType.Dragon, Element.Fire, new List<ISpecialty> { new AfraidOfDragons() });
            ICard player2Card = new MonsterCard("WaterDragon", rawDamage2, MonsterType.Dragon, Element.Water, new List<ISpecialty> { new AfraidOfDragons() });
        ...
        {
            ICard player1Card = new MonsterCard("WoodWizard", rawDamage1, MonsterType.Wizard, Element.Wood, new List<ISpecialty> { new ControlOrk() });
            ICard player2Card = new MonsterCard("IceOrk", rawDamage2, MonsterType.Ork, Element.Ice, new List<ISpecialty>());
        ...
        {
            float damage1 = rawDamage1;
            float damage2 = rawDamage2;
        ...
        {
            ICard player1Card = new MonsterCard("IceKnight", rawDamage1, MonsterType.Knight, Element.Ice, new List<ISpecialty> { new DrownByWaterSpell() });
            ICard player2Card = new SpellCard("WaterSpell", rawDamage2, Element.Water, new List<ISpecialty> { new DamageBoost() });

            (float damage1, float damage2) = DamageUtility.CalculateDamage(player1Card, player2Card, _battleLog);
            Assert.Zero(damage1, "Knight gets drowned by WaterSpell, so zero damage");
            Assert.IsTrue(Math.Abs(damage2 - rawDamage2) < 0.01 || Math.Abs(damage2 - rawDamage2 / 2) < 0.01, "WaterSpell is not effective against Ice, so damage is halved but it also has a fifty percent chance to deal double damage");
            Assert.Pass("FireKraken VS WaterSpell passed the test.");
        }
    }
}