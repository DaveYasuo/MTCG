...
namespace MTCG.Handler.RequestHandling
{
    public partial class RequestHandler
    {
        ...
        private static Response PostTransactionPackages(RequestData requestData)
        {
            ...
            var coins = DataHandler.GetUserCoins(username);
            if (coins is -1) return Response.Status(Status.InternalServerError);
            if (coins - packageCost >= 0)
                return DataHandler.AcquirePackage(username, packageCost)
                    ? Response.PlainText("Package acquired", Status.Created)
                    : Response.PlainText("Could not acquire package", Status.BadRequest);
        ...
        private static Response PostTradings(RequestData requestData)
        {
            ...
            if (requestData.PathVariable == null)
                try
                {
                    var deal = JsonSerializer.Deserialize<TradingDeal>(payload);
                    if (deal == null) return Response.Status(Status.BadRequest);
                    if (DataHandler.GetCard(deal.CardToTrade).Username != username)
                        return Response.Status(Status.Forbidden);
                    return DataHandler.AddTradingDeal(username, deal)
                        ? Response.PlainText("Deal added", Status.Created)
                        : Response.PlainText("Deal cannot be added", Status.Forbidden);
                }
                catch (Exception e)
                {
                    Console.WriteLine(e);
            ...
            try
            {
                var dealId = requestData.PathVariable;
                var cardOfStore = DataHandler.GetCardOfStore(dealId);
                if (cardOfStore == null) return Response.Status(Status.BadRequest);
                var tradeCardId = JsonSerializer.Deserialize<string>(payload);
                var tradeCard = DataHandler.GetCard(tradeCardId);
                if (tradeCard == null || tradeCard.Username != username) return Response.Status(Status.Forbidden);
                if (!(tradeCard.Damage >= cardOfStore.MinimumDamage) ||
                    tradeCard.GetCardType() != cardOfStore.GetCardType())
                    return Response.PlainText("Trade requirements not met");
                var dealCard = DataHandler.GetCard(cardOfStore.CardToTrade);
        ...
        {
            var username = requestData.Authentication.Username;
            var token = requestData.Authentication.Token;
            if (username is null) return Response.Status(Status.BadRequest);
            ...
            if (!_auth.UpdateStatus(token, UserStatus.Occupied, UserStatus.Available))
                return Response.PlainText("User already in game");
            try
            {
                var result = _game.Play(username, cards);
        ...
        {
            var credentials = DataHandler.GetCredentials(data.Authentication.Username);
            ...
            if (!data.Authentication.Username.Equals("admin") || credentials.Role != Role.Admin ||
                !data.Authentication.Token.Equals(credentials.Token)) return Response.Status(Status.Forbidden);
            var payload = data.Payload;
            if (string.IsNullOrEmpty(payload)) return Response.Status(Status.BadRequest);
            try
            {
                var package = JsonSerializer.Deserialize<List<Card>>(payload);
                if (package is not { Count: 5 }) return Response.Status(Status.BadRequest);
                return DataHandler.AddPackage(package)
                    ? Response.PlainText("Package added", Status.Created)
                    : Response.PlainText("Failed to create package", Status.BadRequest);
            }
            catch (Exception e)
            {
                _log.WriteLine(e.Message);
        ...
        public Response PostUser(RequestData requestData)
        {
            ...
            try
            {
                ...
                var user = JsonSerializer.Deserialize<User>(payload);
                if (user == null) return Response.Status(Status.BadRequest);
                return _auth.Register(user)
                    ? Response.PlainText("Register successful", Status.Created)
