...
namespace MTCG.Handler
{
    public static class DataHandler
    {
        ...
        public static bool AddUser(Credentials auth)
        {
            ...
            var userProfile = new Profile(auth.Username);
            ...
            try
            {
                ...
                using var cmd = new NpgsqlBatch(conn, transaction)
                {
                    BatchCommands =
                    {
        ...
        {
            using var conn = Connection();
            using var cmd = new NpgsqlCommand("SELECT token FROM credentials WHERE token=@p1", conn);
        ...
        public static bool AddTokenToUser(string token, string username)
        {
            ...
            {
                int affectedRows;
                using (var cmd = new NpgsqlCommand("UPDATE credentials SET token=@p1 WHERE username=@p2",
                    conn, transaction))
                {
                    cmd.Parameters.AddWithValue("p1", token);
        ...
        /// <summary>
        ///     Get Profile object from one user in the database. Contains of username, name, bio, image, elo, wins, losses, draws,
        ///     coins.
        /// </summary>
        /// <param name="username"></param>
        ...
        public static long GetUserCoins(string username)
        {
            ...
            try
            {
                using var cmd = new NpgsqlCommand("SELECT coins FROM profile WHERE username=@p1", conn);
                cmd.Parameters.AddWithValue("p1", username);
                cmd.Prepare();
                using var reader = cmd.ExecuteReader();
                if (!reader.Read()) return -1;
                var coins = reader.SafeGet<long>("coins");
        ...
        public static bool AcquirePackage(string username, int packageCost)
        {
            ...
            var coins = GetUserCoins(username);
            if (coins is -1 || coins - packageCost < 0) return false;
            ...
            try
            {
                ...
                using (var profileCmd = new NpgsqlCommand(coinsSql, conn, transaction))
                {
                ...
                var packageId = (Guid)pkIdResult;

                // Update all cards with the associated package id: add username to the cards
                using (var cardCmd = new NpgsqlCommand("UPDATE cards SET username=@p1 WHERE package=@p2",
                    conn, transaction))
                {
        ...
        public static bool UpdateDeck(List<Guid> deck, string username)
        {
            ...
            try
            {
                ...
                {
                    using var checkCard = new NpgsqlCommand("SELECT id FROM store WHERE card_id=@p1", conn);
        ...
        /// <summary>
        ///     Get the top player as a List and also add the surrounding player of user at the end of the list, if user is not on
        ///     the top
        /// </summary>
        /// <param name="username"></param>
        ...
        {
            using var conn = Connection();
            var store = new List<TradingDeal>();
            try
            {
                using var cmd = new NpgsqlCommand("SELECT id, card_id, type, damage FROM store", conn);
                cmd.Prepare();
                using var reader = cmd.ExecuteReader();
                while (reader.Read())
                {
        ...
        public static TradingDeal GetCardOfStore(string id)
        {
            ...
            try
            {
                using var cmd = new NpgsqlCommand("SELECT id, card_id, type, damage FROM store WHERE id=@p1", conn);
                cmd.Parameters.AddWithValue("p1", idGuid);
                cmd.Prepare();
                using var reader = cmd.ExecuteReader();
                if (!reader.Read()) return null;
                var card = new TradingDeal(
                    reader.SafeGet<Guid>("id"),
                    reader.SafeGet<Guid>("card_id"),
                    reader.SafeGet<string>("type"),
                    reader.SafeGet<float>("damage")
        ...
        {
            var idGuid = Guid.Parse(storeId);
            using var conn = Connection();
            using var transaction = conn.BeginTransaction();
            try
            {
                using var cmd = new NpgsqlCommand("DELETE FROM store WHERE id=@p1", conn, transaction);
                cmd.Parameters.AddWithValue("p1", idGuid);
                cmd.Prepare();
                if (cmd.ExecuteNonQuery() == 1)
                {
